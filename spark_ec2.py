...
# configuration to support main node running as spot-instance. Please
# see "--main-spot-price" setting.
#
# San-Chuan Hung (2015)
...
def parse_args():
    ...
    parser.add_option(
        "--main-spot-price", metavar="MASTER_PRICE", type="float",
        help="If specified, launch main as spot instances with the given " +
             "maximum price (in dollars)")
    parser.add_option(
...
def launch_cluster(conn, opts, cluster_name):
    ...
    if existing_mains:
        ...
        if opts.main_spot_price is not None:
            # Launch spot instances with the requested price
            print("Requesting main as spot instances with price $%.3f" %
                  (opts.main_spot_price))
            ...
            my_req_ids += [req.id for req in main_reqs]
            ...
            try:
                while True:
                    ...
                    if len(active_instance_ids) == 1:
                        print("Main is granted")
                        reservations = conn.get_all_reservations(active_instance_ids)
                        for r in reservations:
                            main_nodes = r.instances
                        break
                    else:
                        print("Main still wait for granted.")
            except:
                print("Canceling spot instance requests")
                conn.cancel_spot_instance_requests(my_req_ids)
                # Log a warning if any of these requests actually launched instances:
