...
def parse_args():
    ...
    parser.add_option(
        "--master-spot-price", metavar="MASTER_PRICE", type="float",
        help="If specified, launch master as spot instances with the given " +
             "maximum price (in dollars)")
    parser.add_option(
            ...
            try:
                while True:
                    ...
                    if len(active_instance_ids) == 1:
                        print("Master is granted")
                        reservations = conn.get_all_reservations(active_instance_ids)
                        for r in reservations:
                            master_nodes = r.instances
                        break
                    else:
                        print("Master still wait for granted.")
            except:
                print("Canceling spot instance requests")
                conn.cancel_spot_instance_requests(my_req_ids)
                # Log a warning if any of these requests actually launched instances:
